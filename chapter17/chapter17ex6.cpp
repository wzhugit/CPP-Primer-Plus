...
std::ostream& operator<<(std::ostream& os, const manager& man)
{
	os << (abstr_emp&)man;
	os << "The number of the staffs that manager is in charge of is " << man.inchargeof << std::endl;
	return os;
}
...
std::ostream& operator<<(std::ostream& os, fink& fin)
{
	os << (abstr_emp&)os;
	os << fin.reportsto;
...
std::ostream& operator<<(std::ostream& os, highfink& hf)
{
	os << (manager&)hf;
	os << (fink&)hf;
